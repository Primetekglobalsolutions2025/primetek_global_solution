...
export default function AnalyticsCharts({ attendanceData, applicationData }: AnalyticsChartsProps) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      {/* Attendance Trends */}
      <Card hover={false} className="p-5 rounded-xl border border-border shadow-sm overflow-hidden relative">
        <div className="absolute top-0 right-0 p-6 opacity-[0.02] -mr-6 -mt-6">
        ...
        <div className="relative">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="font-semibold text-navy-900 text-sm tracking-tight">Attendance Consistency</h3>
              <p className="text-[11px] text-text-muted font-medium mt-0.5">Average daily check-in percentage</p>
            </div>
            ...
            {attendanceData.map((point, i) => (
              <div key={point.label} className="flex-1 flex flex-col items-center group">
                <div className="relative w-full flex flex-col items-center">
                  ...
                  <div className="absolute -top-7 opacity-0 group-hover:opacity-100 transition-opacity bg-navy-900 text-white text-[9px] px-1.5 py-0.5 rounded font-bold whitespace-nowrap pointer-events-none">
                    {point.value}%
                  </div>
                </div>
                <span className="text-[9px] font-semibold text-text-muted mt-2 uppercase tracking-wider truncate w-full text-center">
                  {point.label}
                </span>
              </div>
            ))}
      ...
      <Card hover={false} className="p-5 rounded-xl border border-border shadow-sm overflow-hidden relative">
        ...
        <div className="relative">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="font-semibold text-navy-900 text-sm tracking-tight">Recruitment Velocity</h3>
              <p className="text-[11px] text-text-muted font-medium mt-0.5">Applications received per week</p>
            </div>
